...
add('root version 0.1.17', pkg.version === '0.1.17');
add('client version 0.1.17', clientPkg.version === '0.1.17');
add('server version 0.1.17', serverPkg.version === '0.1.17');
add('health version 0.1.17', api.includes("version: '0.1.17'"));
add('map parity script points to v0.1.17', pkg.scripts?.['check:map-parity'] === 'node tools/validate-v0117.mjs');
...
add('MapLibre missing sprite ids resolved locally', map.includes('resolveMissingStyleImage') && map.includes('new Uint8Array(2 * 2 * 4)'));
add('globe pointer zoom avoids unsupported around easing', map.includes('syncProjectionZoomInteractions') && map.includes('map.scrollZoom?.disable()') && map.includes('onGlobeWheel'));
add('catalog primary-to-mirror failover exists', catalog.includes('CELESTRAK_MIRROR_BASE') && catalog.includes("provider = 'Satvisor mirror'") && catalog.includes("loadedFrom = 'network-mirror'"));
add('provider HTML is sanitized', catalog.includes('sanitizeProviderText') && api.includes('safeServiceMessage'));
